...
@MainActor
@available(iOS 13.0, *)
class CloudDocument<FilePresenter>: UIDocument where FilePresenter: CloudFilePresenter   {
    
    private weak var documentModel: CloudFileModel<FilePresenter>?
    
    /// All versions of conflicts that need to be resolved in the current document.
    @AtomicValue(.NSLock, defaultValue: [])
    private(set) var conflictedVersions:   [ConflictedVersion<FilePresenter>]
    ...
    @AtomicValue(.NSLock, defaultValue: FilePresenter.createEmptyPresenter())
    var content: FilePresenter
    ...
    func setDocumentModel(for model: CloudFileModel<FilePresenter>) {
    ...
    private func fetchConflictVersion() -> [ConflictedVersion<FilePresenter>] {
    ...
    @MainActor
    func selectVersion(for version: ConflictedVersion<FilePresenter>) throws {
        do {
...
//MARK: - UIDocument Override
    ...
    public override func contents(forType typeName: String) throws -> Any {
        return try self.content.getPresentedData()
    }
    ...
    public override func load(fromContents contents: Any, ofType typeName: String?) throws {
        ...
        self.fileData = data
        self.content = try .createPresenter(from: data)
    }
...
@MainActor
@available(iOS 13.0, *)
public struct ConflictedVersion<FilePresenter>: Identifiable where FilePresenter: CloudFilePresenter {
    public let id = UUID()
    ...
    public var content: FilePresenter {
        get throws {
            let doc = CloudDocument<FilePresenter>(fileURL: self.url)
